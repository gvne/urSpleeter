...
spleeter::Waveform read(const String &file, double *sample_rate, std::error_code& err) {
...
void resample(spleeter::Waveform& source, double original, double desired) {
  ...
  spleeter::Waveform output(2, static_cast<int>(floor(source.cols() * speed_ratio + 1)));
  ...
  for (auto channel_idx = 0; channel_idx < source.rows(); channel_idx++) {
    interpolator.process(speed_ratio, input_channel.data(), output_channel.data(), output_channel.cols());
...
void write(const String& output_path, const spleeter::Waveform& data) {
  ...
  auto writer = std::unique_ptr<AudioFormatWriter>(format.createWriterFor(output_file.createOutputStream(), 44100, data.rows(), 16, StringPairArray(), 0));
